...
\begin{proof}
First, for the finite horizon $T < \infty$, applying Ville's maximal inequality to the nonnegative supermartingale $(E_{1:t})_{0\le t\le T}$ yields the first inequality:
\begin{equation}
 \Pbb_0\left(\max_{0\leq t\leq T}E_{1:t}\geq \frac{1}{\alpha}\right)\leq \alpha.
\end{equation}
The logarithmic version follows immediately from the monotonicity of the logarithm.
Second, for any stopping time $\tau$ (possibly infinite), the stopped index $\tau\wedge T$ is a bounded stopping time since $\tau\wedge T \leq T < \infty$. Optional stopping for bounded nonnegative supermartingales therefore applies directly to $(E_{1:t})_{0\le t\le T}$, yielding $\E_0(E_{1:\tau\wedge T})\leq \E_0(E_{1:0})=1$. This proves that the stopped process at any stopping time remains a valid e-value. (Note that if an infinite-horizon validation process were defined on an infinite filtration, the same result would hold for any stopping time $\tau$ via Fatou's lemma on the nonnegative supermartingale.)
\end{proof}
...
\begin{proof}
For each fixed $\theta\in\Theta_0$, $p_\theta(y\mid\calF_{t-1})\leq p_t^{\mathrm{env}}(y\mid\calF_{t-1})$ pointwise. Hence
\begin{align}
 \E_\theta(\overline E_t\mid\calF_{t-1})
 &=\int \frac{q_t(y\mid\calF_{t-1})}{p_t^{\mathrm{env}}(y\mid\calF_{t-1})}p_\theta(y\mid\calF_{t-1})\mu_t(\mathrm dy)\\
...
\centering
\footnotesize
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.03\linewidth}>{\raggedright\arraybackslash}p{0.14\linewidth}>{\raggedright\arraybackslash}p{0.18\linewidth}>{\raggedright\arraybackslash}p{0.18\linewidth}>{\centering\arraybackslash}p{0.03\linewidth}>{\centering\arraybackslash}p{0.05\linewidth}>{\centering\arraybackslash}p{0.04\linewidth}>{\centering\arraybackslash}p{0.04\linewidth}>{\centering\arraybackslash}p{0.08\linewidth}>{\raggedleft\arraybackslash}p{0.14\linewidth}@{}}
\toprule
Sc. & Null gen. $p_0$ & Data generator & Diagnostic $q_t$ & $K_0$ & $K_{\mathrm{true}}$ & $T$ & $n_{\mathrm{rep}}$ & Threshold & Crossing rate (SE) \\
\midrule
...
\label{sec:comparison_pseudo_residuals}To evaluate the practical utility and power of the proposed predictive e-diagnostics, we compare them against classical diagnostic tests based on pseudo-residuals. It is important to distinguish componentwise marginal probability integral transform (PIT) residuals from joint sequential Rosenblatt residuals. Under a correctly specified movement HMM, the sequential PIT residuals computed from the full one-step conditional predictive distribution for step lengths ($u_{t,\text{step}}$) and turning angles ($u_{t,\text{angle}}$) individually form sequences of independent $\text{Uniform}(0,1)$ variables (subject to continuity). However, these componentwise marginal PITs do not capture contemporaneous dependence between the two dimensions within the same step. To obtain fully independent $\text{Uniform}(0,1)$ variables across both dimensions and time, one must construct joint sequential Rosenblatt residuals by conditioning turning angles on step lengths (or vice versa), i.e., $U_t^L = F_{0,t}^L(L_t \mid \calF_{t-1})$ and $U_t^\Theta = F_{0,t}^{\Theta\mid L}(\Theta_t \mid L_t, \calF_{t-1})$. These joint residuals can then be mapped to standard normal variables to perform standard tests \citep{zucchini_2016_hmm}. Moreover, while classical tests are retrospective terminal tests evaluated at the end of the validation trajectory, the proposed e-diagnostics use targeted alternative models and anytime-valid thresholds, allowing for online sequential monitoring. Because the classical tests are evaluated retrospectively and are not adjusted for sequential or multiple checking, their power is not directly comparable but serves as a useful retrospective benchmark.
