...
The main limitation is the treatment of fitted parameters. The exact guarantee is cleanest when the fitted null and diagnostic alternatives are fixed before validation. This is a strength for a first methodological article because the null statement is transparent: conditional on training, does the fitted HMM generate the validation trajectory credibly? Same-sample exploratory workflows are still useful but require caution. Possible extensions include pre-specified diagnostic menus, cross-fitted averages over independent individuals, sequential sample splitting, composite-null envelopes and simulation-calibrated feature e-values.

The next computational step is to implement a minimal prototype that simulates HMM trajectories, computes observable predictive densities by filtering and verifies the null behaviour of the e-process. Subsequent simulations should target missing states, angular misspecification, residual step-angle dependence, duration misspecification and localized changes in movement behaviour. A real-data application should emphasize not only whether a threshold is crossed, but also when, where and under which predicted behavioural state the evidence accumulates.
